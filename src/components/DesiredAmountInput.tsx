    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] px-3 py-2.5 shadow-[0_4px_16px_rgba(0,0,0,0.2)] focus-within:border-emerald-500/40">
          <input
            ...
            inputMode="decimal"
            autoComplete="off"
            placeholder={messages.amount.placeholder}
            disabled={disabled}
            value={value}
            ...
            aria-label={`${messages.amount.label} ${deliveryAsset}`}
          />
        ...
        {invalidHint ? (
          ...
          <div className="space-y-0.5">
            <p className="text-[10px] text-neutral-600">
              {messages.amount.oracleNote.replace(
                "{buffer}",
                String((ORACLE_CONSERVATIVE_BUFFER * 100).toFixed(0)),
              )}
            </p>
            {quote ? (
              <p className="text-[10px] text-neutral-500">
                {messages.amount.estimatedDelivery}{" "}
      ...
      <div className="rounded-2xl bg-emerald-950/15 p-4 shadow-[0_8px_28px_rgba(0,0,0,0.3)]">
        <p className="text-xs text-neutral-400">
          {messages.amount.estimatedPayment}
          {quote ? <span className="ml-1 text-emerald-500/80">· {messages.amount.live}</span> : null}
        </p>
        {quote ? (
          <>
            <p className="mt-1 text-2xl font-bold tabular-nums text-white">
              {formatPackageUsd(quote.packageUsd)}{" "}
              <span className="text-base font-normal text-neutral-400">{messages.amount.charge}</span>
            </p>
            <p className="mt-2 text-[10px] text-neutral-500">
              ...
              {quote.deliveryAsset} {messages.amount.delivered} · {messages.amount.netGas} $
              {quote.netUsdForGas.toFixed(2)}
            </p>
          </>
        ) : (
          <p className="mt-1 text-sm text-neutral-500">{messages.amount.enterValid}</p>
        )}
      </div>
    </div>
  );
