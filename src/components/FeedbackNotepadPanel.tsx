...
export function FeedbackNotepadPanel({ open, onClose }: FeedbackNotepadPanelProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(typeof body.error === "string" ? body.error : "Gönderilemedi.");
      }
      showToast({
        variant: "success",
        title: "Teşekkürler",
        message: "Mesajınız bize ulaştı — geri bildiriminiz için teşekkür ederiz.",
        persist: true,
      });
      setEmail("");
      setMessage("");
      ...
      >
        ...
        <div className="feedback-notepad-sheet relative overflow-hidden px-7 pb-8 pt-7 sm:px-9 sm:pb-10 sm:pt-8">
          ...
          <p className="mb-5 text-[12px] leading-relaxed text-[#7a6648] sm:text-[13px]">
            Soru, öneri veya hata bildirimi — mesajınız doğrudan ekibe iletilir.
          </p>
          ...
          <textarea
            ...
            placeholder="Geri bildiriminiz veya öneriniz…"
            rows={9}
